...
class ChatRouter(private val huffman: Huffman) {
    ...
    fun send(player: Player, text: String, effects: Int) {
        if (player.chatType == "public" && (text == "/s" || text.startsWith(GLOBAL_PREFIX))) {
            sendGlobal(player, text.removePrefix("/s").trim())
            return
        }
    ...
    private fun sendGlobal(player: Player, text: String) {
        if (text.isEmpty()) {
            player.message("Usage: /s <message>")
            return
        }
